...
def run_rgbd2pcd(depth_root, frgb_root, parse_root, point_dst):
    depth_list = sorted(os.listdir(depth_root))
    for fd_name in tqdm(depth_list):
        ...
        fd_path = os.path.join(depth_root, fd_name)
        bd_path = os.path.join(depth_root, bd_name)
        frgb_path = os.path.join(frgb_root, frgb_name)
        parse_path = os.path.join(parse_root, parse_name)

        os.makedirs(point_dst, exist_ok=True)
        point_out_fn = os.path.join(point_dst, fd_name.replace('_whole_front_depth.npy', '.ply'))

        fd = np.load(fd_path) - 0.02
        bd = np.load(bd_path)
        rm_idx = (fd - bd) < 0
        fd[rm_idx] = 0
        bd[rm_idx] = 0
    ...
    print(f'The unprojected point cloud file(s) are saved to {point_dst}')